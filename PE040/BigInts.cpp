#include "BigInts.h"
   ...
   BigInt  BigInt::sum_digits() const
    ...
    BigInt BigInt::add(int passed)
    {
        BigInt temp=passed;
        int i=0;
        
        for(i=temp._array.size()-1;i>=0;i--)
        {
            _array.push_back(temp._array[i]);         
        }  
    ...
    {
        return _array[_array.size()-INT-1];   
    }
    
    long BigInt::digit_to_long(int INT) const
    {
            return _array[_array.size()-INT-1];
    }
